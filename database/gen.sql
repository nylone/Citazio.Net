...
CREATE TRIGGER SIGNUP_TOKEN_MAX_USAGES BEFORE
INSERT ON USERS FOR EACH ROW BEGIN IF (
        (
            SELECT EXPIRES_AT
            FROM SIGNUP_TOKENS
            WHERE TOKEN_ID = NEW.TOKEN_ID
        ) < CURRENT_TIMESTAMP()
    ) THEN SIGNAL SQLSTATE '10002'
SET MESSAGE_TEXT = 'TOKEN HAS EXPIRED';
ELSEIF (
    (
        SELECT COUNT(*)
        FROM USERS
...
SET MESSAGE_TEXT = 'TOKEN HAS BEEN USED TOO MANY TIMES';
END IF;
END;
...
CREATE OR REPLACE PROCEDURE CAN_EDIT_BOARD(
        IN USER_ID BIGINT UNSIGNED,
        IN BOARD_ID BIGINT UNSIGNED,
        IN REQUIRED_LVL BIT(2)
    ) BEGIN
SELECT IF(
        (
            SELECT OWNER_ID
...
CREATE OR REPLACE PROCEDURE GET_OWN_BOARDS(IN USER_ID BIGINT UNSIGNED) BEGIN
SELECT BOARD_ID,
    PATH,
    NAME
FROM BOARDS B
...
CREATE OR REPLACE PROCEDURE GET_PUBLIC_BOARDS(IN USER_ID BIGINT UNSIGNED) BEGIN
SELECT BOARD_ID,
    PATH,
    NAME
FROM BOARDS B
...
CREATE OR REPLACE PROCEDURE GET_SUBSCRIBED_BOARDS(IN USER_ID BIGINT UNSIGNED) BEGIN
SELECT B.BOARD_ID,
    B.PATH,
    B.NAME,
    B2U.ACCESS_LVL
...
CREATE OR REPLACE PROCEDURE GET_PHC_FROM_USERNAME(IN USERNAME VARCHAR(25)) BEGIN
SELECT PHC
FROM USERS U
WHERE U.USERNAME = USERNAME;
END;

CREATE OR REPLACE PROCEDURE GET_USER_FROM_ID(IN ID BIGINT UNSIGNED) BEGIN
SELECT *
FROM USERS U
WHERE U.USER_ID = ID;
END;

CREATE OR REPLACE PROCEDURE GET_USER_FROM_USERNAME(IN USERNAME VARCHAR(25)) BEGIN
SELECT *
FROM USERS U
WHERE U.USERNAME = USERNAME;
END;
...
CREATE OR REPLACE PROCEDURE ADD_BOARD(
        IN NAME VARCHAR(25),
        IN PATH VARCHAR(25),
        IN OWNER_ID BIGINT UNSIGNED
    ) BEGIN
INSERT INTO BOARDS(OWNER_ID, PATH, NAME) VALUE (OWNER_ID, PATH, NAME);
SELECT LAST_INSERT_ID() AS BOARD_ID;
END;

CREATE OR REPLACE PROCEDURE ADD_BOARD_TO_USER(
        IN EXECUTOR_ID BIGINT UNSIGNED,
        IN BOARD_ID BIGINT UNSIGNED,
        IN USER_ID BIGINT UNSIGNED,
        IN ACCESS_LVL BIT(2)
    ) BEGIN IF CAN_EDIT_BOARD(EXECUTOR_ID, BOARD_ID, 2) THEN
...
CREATE OR REPLACE PROCEDURE ADD_QUOTE(
        ...
        IN BOARD_ID BIGINT UNSIGNED,
        IN USER_ID BIGINT UNSIGNED
    ) BEGIN IF CAN_EDIT_BOARD(EXECUTOR_ID, BOARD_ID, 1) THEN
