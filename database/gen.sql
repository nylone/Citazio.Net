...
CREATE TRIGGER SIGNUP_TOKEN_MAX_USAGES
    BEFORE
        INSERT
    ON USERS
    FOR EACH ROW
BEGIN
    IF (
            (SELECT EXPIRES_AT
             FROM SIGNUP_TOKENS
             WHERE TOKEN_ID = NEW.TOKEN_ID) < CURRENT_TIMESTAMP()
        ) THEN
    ...
    ELSEIF (
            (SELECT COUNT(*)
             FROM USERS
             WHERE TOKEN_ID = NEW.TOKEN_ID) >= (SELECT MAX_USAGES
                                                FROM SIGNUP_TOKENS
...
(
    BOARD_ID   BIGINT UNSIGNED PRIMARY KEY,
...
BEGIN
    INSERT INTO BOARDS(OWNER_ID, BOARD_ID, NAME) VALUE (OWNER_ID, BOARD_ID, NAME);
END;
