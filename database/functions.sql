create or replace function get_user_id(username varchar(32)) returns bigint unsigned
    reads sql data
begin
    return (select u.id from active_users u where u.username = username);
end;

create or replace function get_board_id(path varchar(32)) returns bigint unsigned
    reads sql data
begin
    return (select b.id from active_boards b where b.path = path);
end;
...
begin
    ...
    if (select owner_id from active_boards b where b.id = @haugal_board_id limit 1) = @haugal_user_id then return true;
    ...
    then return true;
    elseif (select public from active_boards b where b.id = @haugal_board_id) = 1 and required_lvl = 0 then return true;
    else return false;
    end if;
end;
...
begin
    return if((select owner_id from active_boards b where b.path = path) = get_user_id(username), true, false);
end;